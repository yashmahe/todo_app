...
async def update_todo(id, text,due_date, is_completed, assigned_to):
    collection.update_one({"id": id}, {"$set": {"text": text, "due_date": due_date, "is_completed": is_completed, "assigned_to":assigned_to}})
    document = collection.find_one({"id": id})
    return document
